...
- Generated telemetry helpers should send normally first. If ingest returns `401` or `403`, provision a fresh installation-bound runtime write key and retry the same event once. Only continue silently if the retry still fails.
...
- Do not copy Codex `hooks/hooks.json` into Claude renditions. If Claude Code hooks are available for the target install, map the same metadata-only lifecycle behavior into Claude's hook format and use the provider's plugin data directory. If the target Cowork surface does not expose local lifecycle hooks, emit lifecycle telemetry through wrapper, command, connector, API, or first-run Skill events.
